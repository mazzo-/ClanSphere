...
function cs_abcode_smileys($name, $keep = 0) {
  ...
  if($abc_count < 3)
    $data['var'] = ['img2' => '', 'pattern2' => '', 'img3' => '', 'pattern3' => ''];
  elseif($abc_count < 6)
    $data['var'] = ['img5' => '', 'pattern5' => '', 'img6' => '', 'pattern6' => ''];
  elseif($abc_count < 9)
    $data['var'] = ['img8' => '', 'pattern8' => '', 'img9' => '', 'pattern9' => ''];
...
function cs_abcode_php($matches) {
  ...
  if(empty($mode)) {
    ...
    $content['source']['content'] = str_replace(['<br />', '<code>', '</code>'], '', $php_code);
...
function cs_abcode_url($matches) {
  ...
  if(strtolower($java) != 'javascript') {
    ...
    return cs_abcode_i([0,'Javascript Links are not allowed']);
  }
}
...
function cs_abcode_eval($matches) {
  ...
  $matches[1] = str_replace(['<?php','<?','?>'], '', $matches[1]);
