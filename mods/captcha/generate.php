...
if(empty($old['captcha_id'])) {
  $captcha_cells = ['captcha_time','captcha_string','captcha_ip'];
  $captcha_save = [cs_time(),$save_hash,$ip];
  cs_sql_insert(__FILE__, 'captcha', $captcha_cells, $captcha_save);
}
else {
  $captcha_cells = ['captcha_time','captcha_string'];
  $captcha_save = [cs_time(),$save_hash];
