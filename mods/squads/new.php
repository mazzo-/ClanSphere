...
if(isset($_POST['submit'])) {
  ...
  if (!empty($_POST['new_clan']) && !empty($clans_pwd)) {
    $cells = ['clans_name', 'clans_short','clans_pwd', 'users_id'];
    $values = [$_POST['new_clan'], $_POST['new_clan'], $clans_pwd, $account['users_id']];
...
if(!empty($error) OR !isset($_POST['submit'])) {
  ...
  $members_cells = ['users_id','squads_id','members_task','members_order','members_admin'];
  $members_save = [$account['users_id'],$getid['squads_id'],$cs_lang['leader'],1,1];
