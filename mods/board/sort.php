...
if (!empty($_GET['delall'])) {
  cs_sql_update(__FILE__, 'board', ['board_order'], [0], 0, 'board_order != 0');
  cs_sql_update(__FILE__, 'categories', ['categories_order'], [0], 0, "categories_mod = 'board'");
  cs_redirect(NULL, 'board', 'sort');
}

if (!empty($_GET['board'])) {
  $board_cells = ['board_order'];
  $board_save = empty($_GET['order']) ? [0] : [cs_sql_escape($_GET['order'])];
  cs_sql_update(__FILE__, 'board', $board_cells, $board_save, cs_sql_escape($_GET['board']));
  cs_redirect(NULL, 'board', 'sort');
}

if (!empty($_GET['cat'])) {
  $board_cells = ['categories_order'];
  $board_save = empty($_GET['order']) ? [0] : [cs_sql_escape($_GET['order'])];
