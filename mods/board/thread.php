...
if($account['access_board'] < $data['thread']['board_access'] AND empty($check_sq) OR empty($check_pw)) {
  ...
  // Update view
  $thread_cells = ['threads_view'];
  $thread_save = [$data['thread']['threads_view'] +1];
  ...
  if(!empty($account['users_id']) AND $data['thread']['threads_last_time'] > $cs_readtime) {
    ...
    if(empty($read_set['read_id'])) {
      $read_cells = ['threads_id','users_id','read_since'];
      $read_save = [$data['thread']['threads_id'],$account['users_id'],$time_now];
      cs_sql_insert(__FILE__, 'read', $read_cells, $read_save);
    } else {
      cs_sql_update(__FILE__, 'read', ['read_since'], [$time_now], $read_set['read_id'], 0, 0);
    }
  }
  ...
  if (!empty($account['users_id'])) {
    ...
    if(isset($_GET['newabo']) AND empty($cs_abo)) {
      $abonements_cells = ['users_id','threads_id'];
      $abonements_save = [$account['users_id'],$id];
          ...
          if(empty($votes_error)) {
            ...
            if ($data['if']['vote_several'] && is_array($voted_election)) {
                foreach ($voted_election as $key => $election) {
                    $votes_save = [$id,$account['users_id'],$time_now,$election,$users_ip,'board'];
                    cs_sql_insert(__FILE__, 'voted', $votes_cells, $votes_save);
                }
            }
            else {
            $votes_save = [$id,$account['users_id'],$time_now,$voted_election,$users_ip,'board'];
            cs_sql_insert(__FILE__, 'voted', $votes_cells, $votes_save);
            }
