...
function cs_cspnews($all = 0) {
  ...
  if(empty($allow_url_fopen)) {
    ...
    if(!empty($id) AND empty($all)) {
      ...
      cs_sql_update(__FILE__, 'options', ['options_value'], [$id], 0, $opt_where);
    }
    else {
      $opt_where = "options_mod = 'clansphere' AND options_name = 'sec_time'";
      cs_sql_update(__FILE__, 'options', ['options_value'], [cs_time()], 0, $opt_where);
      ...
      if(!empty($content)) {
        ...
        if($content[0] > $cs_main['sec_news']) {
          ...
          cs_sql_update(__FILE__, 'options', ['options_value'], [$sec_news], 0, $opt_where);
        }
